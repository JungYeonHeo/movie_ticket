...
<fmt:parseDate value="${movieInfor.opening_date}" var="date" pattern="yyyy-MM-dd" scope="page" />
<!DOCTYPE html>
<html lang="ko">
<head>
...
<body>
    <div id="wrap">
        ...
        <div class="summary" id="poster" style="background-image: url(${movieInfor.poster_img});"></div>
		<div class="summary-contents">
			<img class="summary-main-poster" src="${movieInfor.poster_img}" alt="영화 포스터" />
			<div class="summary-contents-infor">
				<div class="summary-contents-title">
					<c:if test="${movieInfor.g_rate_id == 1}">
						<c:set var="grate" value="전체관람가"/>
              			<img class="summary-contents-title-grate-img" src="resources/images/전체.png" alt="관람등급이미지">
         			</c:if>
         			<c:if test="${movieInfor.g_rate_id == 2}">
         				<c:set var="grate" value="12세이상관람가"/>
              			<img class="summary-contents-title-grate-img" src="resources/images/12.png" alt="관람등급이미지">
            		</c:if>
            		<c:if test="${movieInfor.g_rate_id == 3}">
            			<c:set var="grate" value="15세이상관람가"/>
              			<img class="summary-contents-title-grate-img" src="resources/images/15.png" alt="관람등급이미지">
            		</c:if>
            		<c:if test="${movieInfor.g_rate_id == 4}">
            			<c:set var="grate" value="청소년관람불가" />
              			<img class="summary-contents-title-grate-img" src="resources/images/청불.png" alt="관람등급이미지">
            		</c:if>
				...
				<div class="summary-contents-etc"><c:out value="${grate}"/> | <fmt:formatDate value="${date}" pattern="yyyy.MM.dd"/> | <c:out value="${genre}"/> | ${movieInfor.time}</div>
				<div class="summary-contents-review">
					<div class="summary-contents-review-gen-avg">
						<img class="summary-contents-review-star-img" src="resources/images/ic-star-small-on.png" alt="별모양" />
		        	...
		        	<div class="detail-contents-crew">
		        		...
		        		<div class="detail-contents-group">
							...
							<c:if test="${fn:length(movieActorList) != 0}">
				        		...
				        		<div class="detail-contents-sub"><c:out value="${actorname}"/></div>
							</c:if>
		        		</div>
		        		<div class="detail-contents-group">
		       	...
		       	<div class="detail-contents">
	        		<div class="detail-contents-title">감독/프로듀서</div>
	        			...
	        			<c:if test="${fn:length(movieDirectorList) != 0}">
			        		...
			        		<div class="detail-contents-crew-group">
			        			<img class="detail-contents-crew-img" src="${list.director_img}" alt="배우 이미지" />
					...
					<div class="detail-contents-crews">
						...
						<c:if test="${fn:length(movieActorList) != 0}">
			        		...
			        		<div class="detail-contents-crew-group">
			        			<img class="detail-contents-crew-img" src="${list.actor_img}" alt="배우 이미지" />
			        			<div class="detail-contents-crew-infor">
		       	...
		       	<div class="detail-contents">
	       			...
	       			<c:forEach items="${score_count}" var="array">
		       			<div style="display: flex;">
			       			...
			       			<div class="review-font"><c:out value="${index}"/></div>
			       			<c:choose>
								<c:when test="${array != 0}">
					       			<fmt:parseNumber var="per" integerOnly="true" value="${array / fn:length(reviewList) * 100}"/>
	       			...
	       			<div class="detail-contents-title">감상평 <span class="review-count">총 ${fn:length(reviewList)}개</span></div>
	       				...
	       				<c:if test="${fn:length(reviewList) != 0}">
			        		...
			        		<div class="detail-contents-review-group">
			        			<div class="detail-contents-review-id">${list.member_id}</div>
			        			<div class="detail-contents-review">
				        			<div class="detail-contents-review-up">
				        				...
				        				<div style="width: 85%; position: relative;">
					        				...
					        				<div class="detail-contents-review-sympathy" style="display: flex; position: absolute; right: 0; bottom: 0;">
						        				<img class="review-like-img" src="resources/images/btn-thumb-up-nor.svg" alt="추천" />
						        				<div class="detail-contents-review-like">공감 <span class="like-count">${list.recommend_count}</span></div>
				        					</div>
				        				</div>
				        			</div>
		       		...
		       		<c:if test="${fn:length(movieImageList) != 0}">
						<div class="detail-contents-stillcut-group">
							<c:forEach items="${movieImageList}" var="list">
								<img class="detail-contents-stillcut-image" src="${list}" alt="스틸컬이미지"/>
							</c:forEach>
		        		</div>
	        		</c:if>
		        </div>
